...
var maxX = 5000;
var minY = 0;
var maxY = 100; // 특정 국가 추적하면 값이 count별로 바뀌고, scatter 이미지 그릴 때 참조해야하므로 일단 전역 변수로
// 2바퀴째 이상 그릴 때에도 똑같아야 하므로 axisSetting의 min max를 직접 변경하지 않고 따로 변수 마련함

var isPursuitChanged = false; // 추적 국가 바뀌었을 때 표시
var pursuitCountries1 = ["China", "China", "Japan", "France", "USA", "Switzerland", "", "", "", ""]; // 추적국가 변경 또는 해제했다가 디폴트로 돌아갈 때 참조
var pursuitCountries2 = ["", "", "Iran", "", "", "", "", "", "", ""];
...
var axisSettings = {// 축 설정 방법 2가지 중 선택하기
    axisIdx : 0,
    pursuitNat1 : ["China", "China", "Japan", "France", "USA", "Switzerland", "", "", "", ""], // 1말, 2초, 2말, 3초, 3말, 4초, 4말, 5초, 5말, 6초
    pursuitNat2 : ["", "", "Iran", "", "", "", "", "", "", ""],
    ...
    defaultMinX : [200, 10000, 20, 100, 2000, 5000, 5000, 5000, 20000, 25000], // 1말, 2초, 2말, 3초, 3말, 4초, 4말, 5초, 5말, 6초
    defaultMaxX : [5000, 50000, 100, 300, 14000, 25000, 25000, 30000, 100000, 50000], // 1월 디폴트 값은 전역변수에서 시작하므로 바꿀시 같이 수정할 것
    defaultMinY : [0, 200, 1, 0, 30, 200, 200, 200, 300, 500],
    defaultMaxY : [100, 1000, 5, 5, 200, 1200, 1000, 700, 1700, 2000],
    intervalMinX : [4800, 40000, 80, 200, 12000, 20000, 20000, 25000, 80000, 25000], // 간격이 너무 좁혀지거나 멀어지지 않도록 쓰는 숫자 (일단 maxX - minX로 채움)
    intervalMinY : [100, 800, 4, 5, 170, 1000, 800, 500, 1400, 1500],
    screenLimitX : [67000, 67000, 80000, 81000, 200000, 660000, 1100000, 1500000, 2000000, 2500000], // 미니맵 최대 수치 // 가장 마지막 값은 추후 축 설정 없이 추적 가능하도록 큰 값 
    screenLimitY : [1600, 1600, 3000, 3300, 12500, 33000, 65000, 90000, 110000, 150000],
...
function drawCircle() {
    ...
    for(let i = 0; i < worldLength; i++) {
        ...
        if ( cordX < 0.1 && cordY < 0.1 ) continue; // 왼쪽 구석에 몰리는 경우가 많아 느려짐. 그리지 않게 함
            ...
            if(prop == "index") {
                ...
                for(let i=0; i<valueLength ; i+=days ){ // 국가가 일정 날짜 간격으로 있으므로 건너뜀
                    if(formers.includes(value[i][0])) {
                        formerNations.push([value[i][0], i]);
                        i += formersGap;

                    } else {
                        nations.push([value[i][0], i]);
                    }
                }
                addNations();
            }
