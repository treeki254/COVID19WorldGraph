...
var pursuitCountries1 = ["China", "China", "Japan", "", "", "", "", "", "", ""]; // 추적국가 변경 또는 해제했다가 디폴트로 돌아갈 때 참조
var pursuitCountries2 = ["", "", "", "", "", "", "", "", "", ""];
...
var axisSettings = {// 축 설정 방법 2가지 중 선택하기
    axisIdx : 0,
    pursuitNat1 : ["China", "China", "Japan", "", "", "", "", "", "", ""], // 1말, 2초, 2말, 3초, 3말, 4초, 4말, 5초, 5말, 6초
    pursuitNat2 : ["", "", "", "", "", "", "", "", "", ""],
    ...
    defaultMinX : [200, 10000, 0, 0, 0, 500, 0, 500, 1000, 0], // 1말, 2초, 2말, 3초, 3말, 4초, 4말, 5초, 5말, 6초
    defaultMaxX : [5000, 50000, 40, 150, 1000, 4000, 3000, 3000, 18000, 40000], // 1월 디폴트 값은 전역변수에서 시작하므로 바꿀시 같이 수정할 것
    defaultMinY : [0, 200, 0, 0, 0, 0, 0, 0, 0, 0],
    defaultMaxY : [100, 1000, 5, 10, 80, 25, 50, 120, 150, 1200],
    intervalMinX : [4800, 40000, 40, 150, 1000, 3500, 3000, 2500, 17000, 40000], // 간격이 너무 좁혀지거나 멀어지지 않도록 쓰는 숫자 (일단 maxX - minX로 채움)
    intervalMinY : [100, 800, 5, 10, 80, 25, 50, 120, 150, 1200],
    screenLimitX : [67000, 67000, 80000, 81000, 82000, 83000, 120500, 150000, 200000, 400000], // 미니맵 최대 수치 // 가장 마지막 값은 추후 축 설정 없이 추적 가능하도록 큰 값 
    screenLimitY : [1600, 1600, 3000, 3300, 3500, 5000, 6100, 7000, 8000, 15000],
            ...
            if(prop == "index") {
                ...
                for(let i=0; i<valueLength ; i+=days ){ // 국가가 일정 날짜 간격으로 있으므로 건너뜀
                    if(formers.includes(value[i][0])) {
                        if (value[i][2] == "Asia") formerNations.push([value[i][0], i]);
                        i += formersGap;

                    } else {
                        if (value[i][2] == "Asia") nations.push([value[i][0], i]);
                    }
                }
                addNations();
            }
